...
//处理小时记录
void SetEixtHTData(int carTypeIndex,int carKindIndex,float TollFare)
{   
	int pos ;
	int sumpos;
	pos=charstoint(CarKindRecord[carKindIndex].PosCount,sizeof(CarKindRecord[carKindIndex].PosCount));
	sumpos=charstoint(CarKindRecord[carKindIndex].PaidSum,sizeof(CarKindRecord[carKindIndex].PaidSum));
	...
	{
		pos=pos-1;
		AddField(GetMsgExitHt()->CarInfoContext[pos].CarTypes[carTypeIndex].CarType,sizeof(GetMsgExitHt()->CarInfoContext[ pos].CarTypes[carTypeIndex].CarType));
		...
		if (0!=GetG_Current_Handl_Charge()   && 0!=sumpos  )
		{
			sumpos=sumpos-1;
			AddFieldAtValue(GetMsgExitHt()->CarInfoContext[sumpos].CarTypes[carTypeIndex].CarType ,sizeof(GetMsgExitHt()->CarInfoContext[ sumpos].CarTypes[carTypeIndex].CarType),GetG_Current_Handl_Charge());
			AddFieldAtValue(GetMsgExitHt()->CarInfoContext[sumpos].Kind_Total ,sizeof(GetMsgExitHt()->CarInfoContext[ sumpos].Kind_Total),GetG_Current_Handl_Charge());
		}
		Sync_ExitHT();
	}
...
//处理下班记录 
void SetExitESData(int carTypeIndex,int carKindIndex,float TollFare)
{   
	...
	pos=charstoint(CarKindRecord[carKindIndex].PosCount,sizeof(CarKindRecord[carKindIndex].PosCount));
	sumpos=charstoint(CarKindRecord[carKindIndex].PaidSum,sizeof(CarKindRecord[carKindIndex].PaidSum));
	NewCarKindSite  =charstoint(CarKindRecord[carKindIndex].NewCarKindSite,sizeof(CarKindRecord[carKindIndex].NewCarKindSite));
	...
	{
		pos=pos-1;
		AddField(MsgExitES->CarInfoContext[pos].CarTypes[ carTypeIndex].CarType,sizeof(MsgExitES->CarInfoContext[pos].CarTypes[ carTypeIndex].CarType));
		...
		{
			sumpos=sumpos-1;
			AddFieldAtValue(MsgExitES->CarInfoContext[sumpos].CarTypes[carTypeIndex].CarType,sizeof(MsgExitES->CarInfoContext[sumpos].CarTypes[carTypeIndex].CarType),GetG_Current_Handl_Charge());
			AddFieldAtValue(MsgExitES->CarInfoContext[sumpos].Kind_Total,sizeof(MsgExitES->CarInfoContext[sumpos].Kind_Total),GetG_Current_Handl_Charge());
		}
		Sync_ExitES();
	}
