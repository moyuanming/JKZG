...
void SendLaneInfo_default(void )
{
    ...
    sprintf(GetMsgLaneInfo()->CarType,"%5.5s",GetG_Current_Handl_Car_Type());
    if (GetG_Current_Handl_Charge()>0)
    {
        sprintf(GetMsgLaneInfo()->Money,"%10d",(int)GetG_Current_Handl_Charge());
    }
    else
    {
        sprintf(GetMsgLaneInfo()->Money,"%10.10s"," ");
    }
    sprintf(GetMsgLaneInfo()->CarKind,"%16.16s",GetG_Current_Handl_Car_Kind());
...
void SendMsgInfo(char* InputMsgType,char* InputException,char* InputPromptMsg)
{
	...
	{
		sprintf(GetMsgMsgInfo()->CarKind ,"%16.9s",GetG_Current_Handl_Car_Kind());
		sprintf(GetMsgMsgInfo()->CarType,"%5.4s",GetG_Current_Handl_Car_Type());
	}
    ...
    if (GetG_Current_Handl_Charge()>0)
    {
        sprintf(GetMsgMsgInfo()->Cash,"%16d",(int)GetG_Current_Handl_Charge());
    }
    else
    {
