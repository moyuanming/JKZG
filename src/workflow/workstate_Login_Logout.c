...
void WorkStation_01_Handl_Input_Number(int nKey)
{
	...
	switch (nKey)
	{
		...
		case VK_9:
			...
			{				
				SetG_NumberByChar(ChangCode(nKey));
...
void WorkStation_02_Handl_Input_Password(int nKey)
{
	...
	switch (nKey)
	{
		...
		case VK_9:
			...
			{				
				SetG_PasswordByChar(ChangCode(nKey));
		...
		case VK_RETURN:
			{
				...
				if (UserExist(GetG_Number(),GetG_Password()))
				{
					...
					else
					{
						...
						{
							LoginSuccess();									
							Set_WorkStation_03_Main_Work2(trim(Getg_szName()),"登陆成功！");
						}
						else
						{
...
void WorkStation_30_SelectWorkMod(int nKey)
{
	...
	{
	case VK_1:			
		if(0==strncmp(GetG_Number(),"555555555",Getg_userNumberLen()))
		{
			UI_Show_Help_Info("此工号不能模拟收费！");
			break;
		}
	...
	case VK_3:
		...
		break;
	case VK_4:
		UI_Show_Help_Info("正在关机，请稍候……");
		system("shutdown -h now");
		break;
	case VK_5:
		UI_Show_Help_Info("正在关闭道软件，请稍候……");
		system("killall emrc_main");
		break;
	case VK_6:
		UI_Show_Help_Info("正在重启车道软件，请稍候……");
		system("reboot ");
		break;
	case VK_7:
