...
int SocketUDP;
int SocketUDPToMe;
int SocketUDPControlLane;
struct sockaddr_in their_SendTOMe_addr;
struct sockaddr_in their_SendControlLane_addr;
...
void Init_UDP_Socket()
{
	...
	SocketUDPControlLane = CreateUDP();
	...
	their_SendControlLane_addr.sin_family = AF_INET;	
	their_SendControlLane_addr.sin_port = htons(GetTCOPort());
	their_SendControlLane_addr.sin_addr.s_addr = inet_addr(GetControlLaneIP());
	bzero(&(their_SendControlLane_addr.sin_zero), 8);
...
void SendUdpToControlPadCard()
{
	struct MSG_TCOCMD  PasdCar;
	memset(&PasdCar,'0',sizeof(struct MSG_TCOCMD));
	PasdCar.TCOType[1]= '2';
	echoic("复合收费发送过车信息:<%s><%s>",GetControlLaneIP(),(char *)&PasdCar);
	UDP_Send(SocketUDPControlLane,(char *)&PasdCar,their_SendControlLane_addr);
}
