...
{	
    ...
    int    svrbilnum;
    int   clientbillnum;
    svrbilnum=  atoi(GetMsgTcoCommand()->Param);	
    clientbillnum  =atoi(GetbillNumber_str());	
    if (svrbilnum>clientbillnum )
    {	
        //如果服务端的票号大于客户端的票号，则以服务端的为准。
        billnum=svrbilnum;
    }
    else//负责， 就以客户端的为准
    {	
        billnum=clientbillnum;
    }
    memset(tempBillNumber,'0',sizeof(tempBillNumber));	
    tempBillNumber[8]=0x00;
    sprintf(tempBillNumber,"%08d",billnum);	
    SetbillNumber_str(tempBillNumber);	
}
...
{   
	...
	else if (0==strncmp(GetMsgTcoCommand()->TCOType,TCOTYPE_TCOFRONT,2))
	{
		echoic("收到复合收费发送过车信息!!!!!!");
		HandleFrontMsg();
	}
