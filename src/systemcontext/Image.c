...
{
    memset((char *)MsgImage,' ',sizeof(struct MSG_IMAGE));
}
...
#ifdef PCMACHINE
	sprintf(TempCapImgNameFullPath,"%c%s%c",imageid,"TEMP.JPG",0x00);
#else
...
void BuildImageFiles(char * msghead,     struct MSG_EXITTR*exittr)
{
	...
	SetImageMsgSubTrans(exittr->DisputeType);
