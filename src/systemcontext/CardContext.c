...
void   Use_CardContext_SVCCard_Pay(void)
{
	...
	tempTollFare=GetG_Current_Handl_Charge();
	...
	else if (_SVRList!=NULL)
	{
		...
		else
		{
			...
			willapy =(float)(int)(GetG_Current_Handl_Charge() * DisRate +0.5);
			...
			if (oldmoney>=0)
			{
				sprintf(charinfo,"%s当前余额:%1.0f元\n应收:%1.0f元,折扣:%1.0f折,实收%1.0f元\n",charinfo,oldmoney,GetG_Current_Handl_Charge(),DisRate*10,willapy);
	...
	if (OK)
	{
		SetG_Current_Handl_Charge(willapy);
		echoic("储值卡");
		if (tempTollFare!=GetG_Current_Handl_Charge())
		{
			sprintf(g_currentPrint.Charge,"%3.0f/%1.0f S",GetG_Current_Handl_Charge(),tempTollFare);
		}
		else
		{
			sprintf(g_currentPrint.Charge,"%3.0f S",GetG_Current_Handl_Charge());
		}
		ShowInformatiom_fee();
...
BOOL Use_CardContext_SVCCard_PayByCard(void)
{
	...
	int BOK=TRUE;
	float writemoney;
	writemoney=GetG_Current_Handl_Charge();
	...
	{
		Setg_DiscountRate((float)GetFareValue(Validate_Car_Type_lane()),GetG_Current_Handl_Charge());
	}
	echoic("返回");
	return BOK;
}
...
void  Use_CardContext_MonthCard_Pay(void)
{
	...
	else
	{
		...
		if (OK &&   atoi(SUBSTRING(_SVRList->Kind))!=atoi(GetG_Current_Handl_Car_Type()))
		{
