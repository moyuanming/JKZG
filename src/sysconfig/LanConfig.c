...
char ControlLane[4];
char ControlLaneIP[20];
void SetControlLane(char * Input)
{
	strncpy(ControlLane,Input,3);
	ControlLane[3] = 0x00;
	memset(ControlLaneIP,0x00,20);
	echoic("SetControlLane = <%s>",Input);
	LoadIniFile(CONTROLLANEIPFILENAME_FILE);   
	strcpy(ControlLaneIP,readItemInIniFile(ControlLane));
	echoic("ControlLaneIP = <%s>",ControlLaneIP);
}
char *GetControlLane()
{
	return ControlLane;
}
char *GetControlLaneIP()
{
	return ControlLaneIP;
}
/******************************************************************************************************
* 从Ini文件中加载车道配置信息
******************************************************************************************************/
...
{   
	...
	SetControlLane(readItemInIniFile("ControlLane")); 
	echoic("装载车道配置完成");
