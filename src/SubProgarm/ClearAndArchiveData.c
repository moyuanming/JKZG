...
static void ClaerFile(const char *InputPath,char *InputFileName)
{
	...
	echocad("ClearAndArchive Exec System :%s",_sysCommand_);
	if (0==system(_sysCommand_))
	{
		echocad("ClearAndArchive 删除%s下的日期为%s的旧文件 ",InputPath,InputFileName);
	}
}
/****************************************************************************************
* 存档数据
...
static void ArchiveFile(const char *InputPath,char *InputFileName)
{
	...
	if (0==system(_sysCommand_))
	{
		echocad("ClearAndArchive 将%s下的日期为%s的旧文件压缩归档到%sArchive",InputPath,InputFileName,InputPath);
	}
}
/****************************************************************************************
...
static int ScanDir( const char *InputPath,const char *InputFileType,const int Days,void (*HandFile)(const char *InputPath,char *InputFileName))
{
	...
	while (NULL != ptr)
	{
		...
		echoic("CurrentDataName:[%s],InputFileType:[%s],DateStart:[%s,%s]\n",CurrentDataName,InputFileType,strstr(CurrentDataName,"20"),GetTimeAddTime(DATETIME_FORMAT_YYYYMMDD,0-Days,ADDTIME_TYPE_DAY));
		if ( NULL != strstr(CurrentDataName,InputFileType) )
		{
			DateStart = strstr(CurrentDataName,"20");//从年开始获取年的字符串
			if ( NULL != DateStart &&
				 strncmp(DateStart,GetTimeAddTime(DATETIME_FORMAT_YYYYMMDD,0-Days,ADDTIME_TYPE_DAY),10) <0)
			{
				echocad("asdfsad  %d",Days );
