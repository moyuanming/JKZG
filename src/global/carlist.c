...
int Car_Insert(int carnum) {
	int c = List_Index(CarList, carnum);
...
PTCar Car_Get(int i) {
	CarListInit();
	PTListItem pli = List_Get(CarList, i);
	if (pli != NULL) {
...
PTAxle Car_GetAxles(PTCar car, int i) {
	if (car != NULL) {
		PTListItem pli = List_Get(car->axles, i);
		if (pli != NULL) {
